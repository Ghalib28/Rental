...
header('location:index.php');
    ...
    // Periksa apakah formulir dikirimkan untuk memperbarui detail halaman
    if(isset($_POST['submit']) && $_POST['submit'] == "Update") {
        // Periksa apakah parameter 'type' diatur dalam URL
        if(isset($_GET['type'])) {
            $pagetype = $_GET['type'];
            $pagedetails = $_POST['pgedetails'];
            
            // Perbarui detail halaman dalam basis data
            $sql = "UPDATE tblpages SET detail='$pagedetails' WHERE type='$pagetype'";
            $query = mysqli_query($koneksidb, $sql);
            $msg = "Data halaman diperbarui dengan berhasil";
        }
    }
}
?>
							...
							<div class="col-md-10">
								<div class="panel panel-default">
									...
									<div class="panel-body">
										...
										<?php if(isset($error) && !empty($error)){?><div class="errorWrap"><strong>ERROR</strong>:<?php echo htmlentities($error); ?> </div><?php } 
                                else if(isset($msg) && !empty($msg)){?><div class="succWrap"><strong>SUCCESS</strong>:<?php echo htmlentities($msg); ?> </div><?php }?>
									...
									<div class="form-group">
										...
										<div class="col-sm-8">
											...
											<?php 
            // Periksa apakah 'type' ada dalam $_GET sebelum mengaksesnya
            $pagetype = isset($_GET['type']) ? $_GET['type'] : '';
            // Jika 'type' ada, ambil detail halaman dari database
            if(!empty($pagetype)) {
                $sql = "SELECT detail from tblpages where type='$pagetype'";
                $query = mysqli_query($koneksidb,$sql);
                // Loop melalui hasil query dan tampilkan detail halaman
